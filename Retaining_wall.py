...
def print_calcs(values):
    if values['job_name'] == 'Retaining_wall':
        values['job_name'] = 'Retaining wall'
    f = open(values['print_location'] + '\\' + values['job_name']+'.txt','w')
    f.write(values['job_name'])
    f.write('\n\nDimensions:')
    f.write('\nRetained height, H: \t\t\t\t'+ values['H'] + '\tmm\nFooting Depth, D: \t\t\t\t' + values['D'] + '\tmm\nLength of toe, Ltoe: \t\t\t\t'+ values['Ltoe'] +'\tmm\tNote: Ltoe spans underneath the wall\n')
    f.write('Length of heel, Lheel: \t\t\t\t'+values['Lheel']+ '\tmm\n')
    f.write('Wall Thickness, tw: \t\t\t\t'+values['tw'] + '\tmm\n')
    f.write('Shear key Depth: \t\t\t\t' + values['SD'] + '\tmm\n')
    f.write('Shear key Width: \t\t\t\t' + values['SW'] + '\tmm\n')
    f.write('Shear key Distance from Toe: \t\t\t'+values['SKD'] + '\tmm\tNote: Distance spans to the beggining of the shear key\n' )
    f.write('\nSoil Properties:\n')
    f.write('Angle of internal friction, : \t\t\t'+ values['Internal_friction']+ '\tDegrees\n')
    f.write('Soil cohesion, c: \t\t\t\t' + values['c'] + '\tKPa\n')
    f.write('Density of soil, :\t\t\t\t' + values['Soil_density'] + '\tKN/m3\n')
    f.write('Allowable bearing capacity: \t\t\t' + values['Allowable_bearing'] + '\tKPa\n')
    f.write('Passive soil coefficient, Kp: \t\t\t' + values['Kp'] + '\n')
    f.write('Active soil coefficient, Ka: \t\t\t' + values['Ka'] + '\n')
    f.write('Angle of Backfill soil: \t\t\t' + values['beta'] + '\tDegrees\n')
    f.write('Overburden Soil: \t\t\t\t' + values['Overburden'] + '\tmm\tNote: Overburden soil acts over the toe not including the area under the wall\n')
    f.write('\nLoads:\n\n')
    f.write('Surcharge: \t\t\t\t\t' + values['Surcharge'] + '\tKPa\n')
    f.write('Additional moment: \t\t\t\t' + values['Additional_moment'] + '\tKNm  Note: Moment acts in same direction as retained soil\n')
    f.write('Additional load ontop of wall: \t\t\t'+ values['Additional_load'] + '\tKN\n')
    f.write('\nResults:\n\n')
    ...
    Dimensions = {'SW': float(values['SW']), 'SD': float(values['SD']), 'SKD': float(values['SKD']),
                  'footing_cover': float(values['footing_cover']), 'H': float(values['H']), 'D': float(values['D']),
                  'tw': float(values['tw']), 'Ltoe': float(values['Ltoe']), 'Lheel': float(values['Lheel']), 'h': h,
                  'wall_cover': float(values['wall_cover'])}

    Results = bearing(Dimensions, float(values['Allowable_bearing']), float(values['DC']),
                                            float(values['Soil_density']), float(values['Surcharge']),
                                            float(values['Additional_moment']), float(values['Ka']),
                                            float(values['Internal_friction']), float(values['c']), float(values['Kp']),
                                            values)
    Results1 = overturning(Dimensions, float(values['Allowable_bearing']), float(values['DC']),
                                                 float(values['Soil_density']), float(values['Surcharge']),
                                                 float(values['Additional_moment']), float(values['Ka']), values)

    f.write('Total Weight on footing, N: \t\t\t'+ str(round(Results['N'],2)) + '\tKN\n')
    f.write('Centroid of N: \t\t\t\t\t' + str(round(Results['N_centroid'],2)) + '\tmm\n')
    #f.write('Moment: \t\t\t\t\t' + str(round(Results['Moment'],2)) + '\tKNm\n')
    f.write('Bearing length, a: \t\t\t\t' + str(round(Results['a'],2)) + '\tm\n')
    f.write('Maximum bearing pressure at SLS, qmax: \t\t' + str(round(Results['qmax'],2)) + '\tKPa')
    if Results['qmax'] > float(values['Allowable_bearing']):
        f.write('\tFail\n')
    else:
        f.write('\tOK\n')
    f.write('Overturning: \t\t\t\t\t' + str(round(Results1,2)) + '\tKNm')
    if Results1 > 0:
        f.write('\tOK\n')
    else:
        f.write('\tFAIL\n')
    f.write('Sliding: \t\t\t\t\t' + str(round(Results['F'],2)) + '\tKN')
    if Results['F'] > 0:
        f.write('\tOK\n')
    else:
        f.write('\tFAIL\n')
...
def bearing(Dimensions,allowable_end_pressure,density_concrete,density_soil,surcharge,Additional_moment,Ka,Internal_friction,c,Kp,values,Top_restraint,Restraint_height):
    ...
    N = density_concrete * Dimensions['H'] * Dimensions['tw'] * 10 ** -6 + (Dimensions['Ltoe']
        ...
        + Dimensions['SW']*Dimensions['SD']*density_concrete*10**-6 + values['Additional_load'] + values['Overburden']*density_soil * (Dimensions['Ltoe'] - Dimensions['tw'])*10**-6
    centroid_N = (density_concrete * Dimensions['H'] * Dimensions['tw'] * 10 ** -6 * (Dimensions['Ltoe'] - Dimensions['tw']/2) + (Dimensions['Ltoe']
                                                                            + Dimensions['Lheel']) * Dimensions[
            'D'] * density_concrete * 10 ** -6 * (Dimensions['Ltoe'] + Dimensions['Lheel'])/2
        + Dimensions['Lheel'] * Dimensions['H'] * density_soil * 10 ** -6 * (Dimensions['Ltoe'] + 0.5*Dimensions['Lheel']) + surcharge * Dimensions['Lheel'] * 10 ** -3 * (Dimensions['Ltoe'] + 0.5*Dimensions['Lheel'])
        + Dimensions['SW']*Dimensions['SD']*density_concrete*10**-6 * (Dimensions['SKD']/2)
                  + values['Additional_load']*(Dimensions['Ltoe'] - Dimensions['tw']/2) + values['Overburden']*density_soil * (Dimensions['Ltoe'] - Dimensions['tw'])**2/2*10**-6)/N
    print(centroid_N,'centroid')

    if Top_restraint == True:
        M_unrestrained = PA1 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/3*10**-3 + PA2 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/2*10**-3 + Additional_moment
        x1 = (Dimensions['H']*10**-3)/(2**0.25)
        thetaB = (Restraint_height/Dimensions['H'])*(Dimensions['H']*10**-3 - x1) * (Ka*density_soil*Dimensions['H']*10**-3)*(Dimensions['H']*10**-3)**3/24
        thetaA = (Restraint_height*10**-3)**3/3
        thetaB2 = Ka*surcharge * (Dimensions['H']*10**-3)**4/8*Restraint_height/(Dimensions['H'])
        M_restrained = (Ka* density_soil * Dimensions['H']*10**-3) * (Dimensions['H']*10**-3)**2/6 - thetaB/thetaA * Restraint_height*10**-3 + Ka * surcharge * (Dimensions['H']*10**-3)**2/2 - thetaB2/thetaA*Restraint_height*10**-3 + Additional_moment

        Moment = M_restrained
        print(M_restrained, 'Moment_R', thetaB,thetaA,x1,thetaB2)
    else:
        Moment = PA1 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/3*10**-3 + PA2 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/2*10**-3 + Additional_moment
    ...
    if Top_restraint == True:
        M_unrestrained = PA1 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/3*10**-3 + PA2 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/2*10**-3 + Additional_moment
        x1 = (Dimensions['H']*10**-3)/(2**0.25)
        thetaB = (Restraint_height/Dimensions['H'])*(Dimensions['H']*10**-3 - x1) * (1.25*Ka*density_soil*Dimensions['H']*10**-3)*(Dimensions['H']*10**-3)**3/24
        thetaA = (Restraint_height*10**-3)**3/3
        thetaB2 = 1.5*Ka*surcharge * (Dimensions['H']*10**-3)**4/8*Restraint_height/(Dimensions['H'])
        M_restrained = (1.25*Ka* density_soil * Dimensions['H']*10**-3) * (Dimensions['H']*10**-3)**2/6 - thetaB/thetaA * Restraint_height*10**-3 + 1.5*Ka * surcharge * (Dimensions['H']*10**-3)**2/2 - thetaB2/thetaA*Restraint_height*10**-3 + Additional_moment

        Moment = M_restrained
        print(M_restrained, 'Moment_R', thetaB,thetaA,x1,thetaB2)
    else:
        Moment = 1.25*PA1 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/3*10**-3 + 1.5*PA2 * (Dimensions['H'] + Dimensions['D'] + Dimensions['h'])/2*10**-3 + Additional_moment
        print(Moment, 'Moment_U')
    ...
    if Top_restraint == True:
        F = 0.9 * (N * math.tan(0.75 * Internal_friction / 180 * math.pi) + c * a_ULS) + 0.9*0.5*Kp*(Dimensions['D']+Dimensions['SD'] + values['Overburden'])**2*density_soil*10**-6
        - (thetaB/thetaA + Ka*density_soil*(Dimensions['H']*10**-3)**2/2)*1.25 - 1.5*(Ka*surcharge*Dimensions['H']*10**-3 - thetaB2/thetaA)
    else:
        F = 0.9 * (N * math.tan(0.75 * Internal_friction / 180 * math.pi) + c * a_ULS) - 1.25 * PA1 - 1.5*PA2 + 0.9*0.5*Kp*(Dimensions['D']+Dimensions['SD'] + values['Overburden'])**2*density_soil*10**-6
    #F = 0.9 * (N * math.tan(0.75 * Internal_friction / 180 * math.pi) + c * a_ULS)
    results = {'qmax':qmax,'F':F,'a_ULS':a_ULS,'qmax_ULS':qmax_ULS,'PA1':PA1,'PA2':PA2,'N':N,'N_centroid':centroid_N,'a':a,'qmax':qmax}
    return results

def overturning(Dimensions,allowable_end_pressure,density_concrete,density_soil,surcharge,Additional_moment,Ka,values,Top_restraint,Restraint_height):
    PA1 = 0.5 * Ka * density_soil * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) ** 2 * 10 ** -6
    PA2 = Ka * surcharge * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) * 10 ** -3
    ...
    M = (density_concrete * Dimensions['H'] * Dimensions['tw'] * (Dimensions['Ltoe'] - Dimensions['tw'] / 2) * 10 ** -9\
          ...
          + Dimensions['Lheel'] * Dimensions['H'] * density_soil * (\
                      Dimensions['Ltoe'] + Dimensions['Lheel'] / 2) * 10 ** -9 + Dimensions['SW'] * Dimensions['SD'] * density_concrete *  (Dimensions['SKD'] + Dimensions['SW']/2)*10**-9\
          ...
          + Dimensions['Lheel'] * Dimensions['h'] * 0.5 * density_soil * (Dimensions['Ltoe'] + Dimensions['Lheel'] * 2 / 3) * 10 ** -9)*0.9\
          - Moment
...
def concrete_wall(Dimensions, allowable_end_pressure, density_concrete, density_soil, surcharge, Additional_moment, Ka,Internal_friction, c,fc,crack,values):
    PA1 = 0.5 * Ka * density_soil * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) ** 2 * 10 ** -6
    PA2 = Ka * surcharge * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) * 10 ** -3
    ...
    while results['Mn']*0.65 < M or results['Mu_min'] > results['Mn']:
        results = Concrete.beam_moment(1000,Dimensions['tw'],Dimensions['wall_cover'],[Dimensions['tw']-Dimensions['wall_cover']],Ast,M,0,0,[0,0,0],fc,[500])
        print(results, 'ast')
        Ast[0] = Ast[0] + 10
    print(results,'ast')
...
def concrete_heel(Dimensions, allowable_end_pressure, density_concrete, density_soil, surcharge, Additional_moment,Ka, Internal_friction, c, fc, crack,values):
    PA1 = 0.5 * Ka * density_soil * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) ** 2 * 10 ** -6
    PA2 = Ka * surcharge * (Dimensions['H'] + Dimensions['D'] + Dimensions['h']) * 10 ** -3
    M = 1.5 * (PA1 * Dimensions['H'] / 3 * 10 ** -3) + PA2 * Dimensions['H'] / 2 * 10 ** -3
...
def concrete_toe(Dimensions, density_soil, surcharge, Additional_moment,Ka, Internal_friction, c, fc, crack, qmax_ULS,a_ULS,values):
...
def masonry_wall(d,fsy,fuc,Masonry,Dimensions,Ka,surcharge,density_soil,values):
    ...
    M = 1.5 * (PA1 * Dimensions['H'] / 3 * 10 ** -3) + PA2 * Dimensions['H'] / 2 * 10 ** -3 + values['Additional_moment']
...
def Soldier(values):
    ...
    if values['cohesion'] > 0:
        Pa = (values['Density_soil'] * values['H'] * values['Ka'] - 2 * values['cohesion'] * math.tan((45 - values['Friction_angle']/2)/180*math.pi)) * (values['H'] - Z) / 2 * values['Spacing']*1.25
    ...
    eq1 = sympy.Eq(x * 2 * values['cohesion'] * math.tan((45 + values['Friction_angle']/2)/180*math.pi) * values[
        'Dia'] * f * 10 ** -3 + x ** 2 * values['Kp'] * values['Density_soil'] * values['Dia'] * f * 10 ** -3 * 0.5 - x*10*(values['H']-values['Water_table']),
                   Pa + Pw + Pwater)
    dn = sympy.solve(eq1)
    d = dn[1]
    print(dn)
    ...
    Mmax = abs(d * 2 * values['cohesion'] * math.tan((45 + values['Friction_angle'] / 2) / 180 * math.pi) * values[
        'Dia'] * f * 10 ** -3 * d / 2 + d * values['Kp'] * values['Density_soil'] * d / 2 * values[
               'Dia'] * f * 10 ** -3 * d / 3 - Pa * ((values['H'] - Z) / 3 + d) - Pw * (values['H'] / 2 + d) - Pwater*((values['H'] - values['Water_table'])/3+d))
